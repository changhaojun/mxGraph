 ...
 * 
 */
BaseFormatPanel.prototype.createCellColorOption = function (label, colorKey, defaultColor, callbackFn, setStyleFn, dataId) {
	var ui = this.editorUi;
	var editor = ui.editor;
	var graph = editor.graph;
	...
	return this.createColorOption(label, function () {
		...
		if(dataId) {
			graph.getSelectionCell().data_id = dataId;
		}
...
StyleFormatPanel.prototype.init = function () {
	...
	this.container.appendChild(this.addAttr());
};
...
StyleFormatPanel.prototype.addAttr = function () {
	...
	if(ss.vertices[0].data_id) {
		input.value = ss.vertices[0].data_id;
	}

	const div_ = document.createElement('div');
	div_.style.margin = '10px 0';
	const btn = mxUtils.button('绑定', () => {
		const val = input.value;
		if(val) {
			input.blur();
			this.createCellColorOption(null, null, null, null,null, val);
		}
	});
	btn.style.width = '100%';
